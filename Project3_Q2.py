...
for k_ind in range(len(k_list)):
    ...
    # Initialization of Centroids k*p*1
    start_centroid_calculation = time.time()    
    C = np.random.rand(k, p).reshape(k, p, 1)
    print("Initialized centroids in: {}", time.time() - start_centroid_calculation)
...
# plt.ylabel("Loss")
# plt.title("K-Means Mini Batch: Number of Iterations vs Loss for different Batch size") 
